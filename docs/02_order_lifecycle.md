...
This resolves the same-ms place+cancel pattern (a common high-frequency order lifecycle) correctly:
...
per-session. Overall session QA results: `reports/SESSION_QA_SUMMARY.txt`;
detailed per-session report generated by `session_qa.py` as `reports/SESSION_QA_REPORT.csv`
(not redistributed — requires downloading raw sessions first).
